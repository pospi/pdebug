...
function __pdebug_show_coverage() {
	...
	foreach ($stats as $file => $lines) {
		if (!preg_match('/^' . addcslashes(dirname(__FILE__), '/\\') . '/', $file)) {
			$fileLines = file($file);
			$out[$file] = array();
	...
	foreach ($out as $header => $data) {
		echo "<h4 onclick=\"PDebug.toggle(this.nextSibling);\">$header <span>(hit " . count($stats[$header]) . " lines " . array_sum($stats[$header]) . " times)</span></h4>";
